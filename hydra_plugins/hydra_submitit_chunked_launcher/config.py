...
@dataclass
class SlurmChunkedQueueConf(BaseQueueConf):
    """Slurm configuration overrides and specific parameters"""

    _target_: str = (
        "hydra_plugins.hydra_submitit_chunked_launcher.chunked_launcher.SlurmChunkedLauncher"
    )
...
@dataclass
class LocalChunkedQueueConf(BaseQueueConf):
    _target_: str = (
        "hydra_plugins.hydra_submitit_chunked_launcher.chunked_launcher.LocalChunkedLauncher"
    )
...
ConfigStore.instance().store(
    ...
    name="submitit_chunked_local",
    node=LocalChunkedQueueConf(),
    provider="submitit_chunked_launcher",
)


ConfigStore.instance().store(
    group="hydra/launcher",
    name="submitit_chunked_slurm",
    node=SlurmChunkedQueueConf(),
    provider="submitit_chunked_launcher",
)